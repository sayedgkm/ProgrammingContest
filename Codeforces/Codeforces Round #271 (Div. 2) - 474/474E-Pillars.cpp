...
void update(int node,int low,int hi,int i,int value){
   ...
   if(low==hi){
    tree[node].sum=max(tree[node].sum,value*1LL); return;
   }
   int mid=(low+hi)/2;
   int left=2*node;
   int right=left+1;
...
int main(){
    ...
    for(int i= n-1;i>=0;i--) {
        ll val = ar[i];
        int l = lower_bound(br,br+n,val+d)-br;
        int mx = 0;
        if(l<n){
            mx = max(mx,query(1,1,rnk,mp[br[l]],rnk));
        }
        l = lower_bound(br,br+n,val-d)-br;
        if(val==7) debug(l,br[l],mp[br[l]]);
        if(abs(br[l]-val)<d) {
            l--;
        }
        if(l>=0)
            mx = max(mx,query(1,1,rnk,1,mp[br[l]]));

        dp[i] = mx+1;
        update(1,1,rnk,mp[val],dp[i]);
        debug(dp[i]);
        ans = max(ans,dp[i]);
