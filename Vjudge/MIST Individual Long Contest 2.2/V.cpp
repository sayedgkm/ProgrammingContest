...
ll query(int u,int v,int n) {
    ...
    while(Chain[u]!=Chain[v]){
        if(Level[ChainLdr[Chain[u]]]<Level[ChainLdr[Chain[v]]]){
            ...
            sum=min(sum,query(1,1,n,Pos[par1],Pos[v]));
            v = Par[ChainLdr[Chain[v]]];
        } else {
            int par1 = ChainLdr[Chain[u]];
            sum=min(sum,query(1,1,n,Pos[par1],Pos[u]));
            u = Par[ChainLdr[Chain[u]]];
        }
    }
   // debug(Pos[u]+1,Pos[v]);
    if( Level[u]<Level[v] ) sum=min(sum,query(1,1,n,Pos[u]+1,Pos[v]));
    else sum=min(sum,query(1,1,n,Pos[v]+1,Pos[u]));

    if(Level[v]<Level[u]) u = v;
    sum=min(sum,query(1,1,n,Pos[u],Pos[u])); /// skip this line if edge update is needed
    return sum;
}
void init() {
    nC = 0;
...
int main(){
    ...
    while(q--) {
        ...
        if(!t){
            if(up[node]==0) {
                up[node] = 1;
                update(1,1,n,Pos[node],Pos[node]);
            } else {
                up[node] =0;
                update(1,1,n,Pos[node],inf);
            }
        } else {
            int val = query(0,node,n);
            if(val==inf) printf("%d\n",-1);
            else printf("%d\n",rPos[val]+1);
        }
    }
