...
ostream &operator<<(ostream &os, const PT &p) {
    os <<setprecision(0)<<fixed<< "(" << p.x << "," << p.y << ")";
}
...
int main(){
    ...
    //cin.tie(0);
   // cout<<(RotateAroundPointCCW(PT(10,0),PT(0,0),90*pi/180))<<endl;
    int test = nxt();
    while(test--) {
        double a = dxt(),b = dxt(),c = dxt(),d = dxt(),e = dxt(),f = dxt();
        PT A(a,b),B(c,d),C(e,f);
        PT CR = ComputeCircleCenter(A,B,C);
        int ans = 3;
        ...
        while(ans<=300) {
            int cnt  = 0;
            for(int i = 0;i<ans;i++) {
                cur=RotateAroundPointCCW(cur,CR,(angle/ans)*pi/180);
                if(cur==A||cur==B||cur==C) cnt++;
            }
            if(cnt==3) break;
            ans++;
        }
